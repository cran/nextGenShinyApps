...
ui <- fluidPage(
  ...
  style = "3",
  #custom.bg.color = "lightgray",

  # Header: Insert header content using titlePanel ------------
  header = titlePanel(left = "A Demo to Showcase the Features of the nextGenShinyApp R package", right = icon("user")),
  ...
  row(
    column(
      ...
      card(
        ...
        # card body content
        tags$h4("Sample text"),
        "Laoreet non curabitur gravida arcu. Placerat orci nulla pellentesque dignissim enim. Viverra accumsan in nisl nisi scelerisque. Convallis a cras semper auctor neque vitae tempus quam. Vestibulum sed arcu non odio euismod. Maecenas accumsan lacus vel facilisis volutpat est velit. Elit pellentesque habitant morbi tristique senectus et netus et malesuada. Erat imperdiet sed euismod nisi porta lorem mollis aliquam ut. Velit scelerisque in dictum non consectetur a. Dui sapien eget mi proin sed libero enim sed faucibus. Ac odio tempor orci dapibus ultrices. Tortor posuere ac ut consequat semper viverra nam libero justo. Tincidunt vitae semper quis lectus nulla. Nisi vitae suscipit tellus mauris. Feugiat in ante metus dictum.",
        tags$br(), tags$br(),
